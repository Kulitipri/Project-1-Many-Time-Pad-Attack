...
# danh sách các ciphertexts (hex-encoded)
...
# xor các bytes của các ciphertexts
def xor_bytes(a: bytes, b: bytes) -> bytes:
    return bytes([a[i] ^ b[i] for i in range(min(len(a), len(b)))])

# kiểm tra byte có phải là ký tự alphabet hay space
def is_alpha_byte(b: int) -> bool:
    return (65 <= b <= 90) or (97 <= b <= 122) or (b == 32)

# dùng threshold để phân tích và lấy key
...
# in kết quả giải ciphertext với key hiện tại
...
# in kết quả giải target ciphertext với key hiện tại
print("\n--- Decryption of target ---")
print(repr(decrypt_with_key(cts[-1], key)))

# thêm các human predictions(cho target ct)
manual_hints += [
    (10, 31, 'n'),
    (10, 34, 'a'),
    (10, 35, ' ')
...
# in kết quả giải ciphertext với key hiện tại (sau khi thêm human predictions)
