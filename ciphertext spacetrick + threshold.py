import binascii

# list of ciphertexts in hex
ct1 = ''
ct2 = '234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f'
ct3 = '32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb'
ct4 = ''
ct5 = '3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070'
ct6 = ''
ct7 = ''
ct8 = '315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3'
ct9 = ''
ct10 = '466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83'
target_ct = '32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904'

# convert hex to bytes
ct_hex = [ct1, ct2, ct3, ct4, ct5, ct6, ct7, ct8, ct9, ct10, target_ct]
cts = [binascii.unhexlify(h) for h in ct_hex]

# xor bytes of ciphertexts
def xor_bytes(a: bytes, b: bytes) -> bytes:
    return bytes([a[i] ^ b[i] for i in range(min(len(a), len(b)))])

#check if byte is alphabet or space
def is_alpha_byte(b: int) -> bool:
    return (65 <= b <= 90) or (97 <= b <= 122) or (b == 32)

#main function to analyze and get key using threshold
def analyze_threshold(threshold=6):
    n = len(cts)
    maxlen = max(len(c) for c in cts)
    counts = [[0]*maxlen for _ in range(n)]
    for i in range(n):
        for j in range(n):
            if i == j: continue
            x = xor_bytes(cts[i], cts[j])
            for pos, val in enumerate(x):
                if is_alpha_byte(val):
                    counts[i][pos] += 1
    likely_spaces = {}
    for i in range(n):
        votes = counts[i]
        spaces = [pos for pos,v in enumerate(votes) if v >= threshold]
        likely_spaces[i] = spaces
    partial_key = {}
    for i, spaces in likely_spaces.items():
        for pos in spaces:
            if pos < len(cts[i]):
                partial_key[pos] = cts[i][pos] ^ 0x20
    return likely_spaces, partial_key

#side function to fill missing key positions using space trick
def fill_key_with_space_trick(key):
    n = len(cts) - 1  # exclude target
    maxlen = max(len(c) for c in cts)
    for i in range(maxlen):
        if i in key:
            continue
        best_count = -1
        best_key = None
        for j in range(n):
            if i >= len(cts[j]):
                continue
            candidate = cts[j][i] ^ 0x20
            count = 0
            for k in range(n):
                if k == j or i >= len(cts[k]):
                    continue
                pt = cts[k][i] ^ candidate
                if is_alpha_byte(pt):
                    count += 1
            if count > best_count:
                best_count = count
                best_key = candidate
        if best_key is not None:
            key[i] = best_key

# function to decrypt with current key
def decrypt_with_key(ct, key):
    return ''.join(chr(ct[pos] ^ key[pos]) if pos in key else '?' for pos in range(len(ct)))

# using threshold to get initial key(adjust threshold here)
likely_spaces, key = analyze_threshold(threshold=6)

#filling missing key positions using space trick
fill_key_with_space_trick(key)

# manual hints 
manual_hints = [ 
    (0, 7, 'f'),
    (0, 18, 'n'),
    (1, 25, 'y'),
    (4, 56, 'p'),
    (3, 2, 'e'),
]
for ci, pos, ch in manual_hints:
    key[pos] = cts[ci][pos] ^ ord(ch)


print("\n--- Decryption of ciphertexts ---")
for i in range(10):
    print(f"Ciphertext {i+1}:")
    print(repr(decrypt_with_key(cts[i], key)))
    print()

print("\n--- Decryption of target ---")
print(repr(decrypt_with_key(cts[-1], key)))

#manual hint with human predictions
manual_hints += [
    (10, 31, 'n'),
    (10, 34, 'a'),
    (10, 35, ' ')
]
for ci, pos, ch in manual_hints:
    key[pos] = cts[ci][pos] ^ ord(ch)

print("\n--- Decryption of target (with human predictions) ---")
print(repr(decrypt_with_key(cts[-1], key)))
